...
Ordering the different genotypes into different groups from the augmented data:
...
#### **Graph 1: Response (pCR vs RD) by ER/PR/HER2 Gentypes**
...
ggplot(dat, aes(x = combo, fill = response)) +
  ...
  labs(
    title = "Response (pCR vs RD) by ER/PR/HER2 Genotypes",
...
#### **Graph 2:** Tumor Grade (1–3) by ER/PR/HER2 Combination
...
Estrogen receptor-positive breast cancer patients have a worse outcome (Graph 1) even tough they do not have a more agressive tumor grade (Graph 2). Being negative for both hormone receptors leads to a more aggressive tumor grade (Graph 2).
...
**Graph 3, heatmap plot, tumor size among genotypes:**
...
Conclusion: far most of the genotype groups have a size 2 tumor stage, or otherwise they have developed into size 3-4. This is especially the case for groups that are Estrogen receptor positive, and the last two groups which are negative for both hormone receptors.
...
**Graph 4: bubble plot, lymph nodes among genotype groups**:
...
Conclusion: generally it seems to be worst to have these three genotypes:
...
**Graph 5 prep: treatment among genotypes**
...
**Graph 5 plot: treatment among genotypes**

```{r}

ggplot(treat_summary, aes(x = combo, y = pct, fill = treatment)) +
  
  # N LABELS ABOVE THE BARS 
  geom_text(
  ...
  coord_cartesian(ylim = c(0, 1.15))   # room for N labels

```

Generally, more different treatments were tried in the last two groups, so in conclusion more personalized treatment options give better response.
